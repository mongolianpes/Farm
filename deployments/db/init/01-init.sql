...
CREATE TABLE messages (
    ...
    create_at timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
    related_announcement_id int references announcements(announcement_id) ON DELETE CASCADE ON UPDATE CASCADE,
    message text NOT NULL
);

CREATE TABLE embeddings_users (
    user_id int references users(user_id) ON DELETE CASCADE ON UPDATE CASCADE,
    text text NOT NULL
);

CREATE TABLE embeddings_announcements (
    announcement_id int references announcements(announcement_id) ON DELETE CASCADE ON UPDATE CASCADE,
    text text NOT NULL
);